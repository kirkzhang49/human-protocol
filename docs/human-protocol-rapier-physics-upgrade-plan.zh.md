...
- `npm run qa:physics:browser`：真实 Chrome browser QA，五个官方关卡均通过 Raw WebGPU backend、browser RAF/game loop、Rapier runtime、console clean 检查；包含 Level 2 移动横屏视口，并断言 Level 1/2 各 1 个动态体、Level 3-5 为 0；同时临时插入薄 cover probe，确认 projectile sweep 的命中点落在墙面附近而不是本帧终点；还临时插入玩家宽/窄门框、玩家高位横梁、玩家可过但 boss 会撞到的架空横梁、连续窄通道 + 旋转家具、敌人 soft/solid 导航障碍、boss 高位横梁和动态 blocker，确认玩家无 filter movement、连续玩家移动、敌人 filtered movement、角色高度与 Phase 7 动态物件碰撞在真实浏览器 runtime 下稳定，并验证角色顶到临时动态 blocker 时会产生可观测水平位移，且 blocker 清理后动态体数量恢复；并对 Level 1/2 的精选动态物件施加 impulse，确认动态体会移动、保持地面高度并同步回 `GameWorld`。
