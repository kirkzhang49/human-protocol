...
- `npm run qa:physics:browser`：真实 Chrome browser QA，五个官方关卡均通过 Raw WebGPU backend、browser RAF/game loop、Rapier runtime、console clean 检查；包含 Level 2 移动横屏视口，并断言 Level 1/2 各 1 个动态体、Level 3-5 为 0；同时临时插入薄 cover probe，确认 projectile sweep 的命中点落在墙面附近而不是本帧终点；还临时插入玩家宽/窄门框和敌人 soft/solid 导航障碍，确认 Phase 4/5 的 kinematic movement/filter 在真实浏览器 runtime 下稳定；并对 Level 1/2 的精选动态物件施加 impulse，确认动态体会移动、保持地面高度并同步回 `GameWorld`。
