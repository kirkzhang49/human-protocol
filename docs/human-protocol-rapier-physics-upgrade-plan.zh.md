...
- `npx vitest run src/game/config/validation/mapValidator.test.ts` 与 `src/game/physics/DynamicPropsIntegration.test.ts`：确认 Phase 7 仍为 opt-in，小物件动态基础层可用，重复 dynamic id 会在进入 Rapier body sync 前被拒绝，官方五关动态化范围被限制在 Level 1 货箱堆和 Level 2 软凳两个精选 ID。
