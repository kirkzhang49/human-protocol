...
- `npm run qa:physics:browser`：真实 Chrome browser QA，五个官方关卡均通过 Raw WebGPU backend、browser RAF/game loop、Rapier runtime、console clean 检查；包含 Level 2 移动横屏视口，并断言 Level 1/2 各 1 个动态体、Level 3-5 为 0；同时临时插入薄 cover probe，确认 projectile sweep 的命中点落在墙面附近而不是本帧终点；还临时插入玩家宽/窄门框、玩家高位横梁、敌人 soft/solid 导航障碍、boss 高位横梁和动态 blocker，确认玩家无 filter movement、敌人 filtered movement、角色高度与 Phase 7 动态物件碰撞在真实浏览器 runtime 下稳定；并对 Level 1/2 的精选动态物件施加 impulse，确认动态体会移动、保持地面高度并同步回 `GameWorld`。
...
- `npx vitest run src/game/systems/CombatRules.test.ts`：覆盖 pulseRifle 近战推动 opt-in 动态物件，以及墙后动态物件不会被隔墙推动。
