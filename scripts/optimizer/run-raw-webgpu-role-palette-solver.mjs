...
const MANIFEST_DIR = path.join(ROOT, "src/assets/manifests/generated/raw-webgpu");
const SOURCE = path.join(ROOT, "scripts/optimizer/raw-webgpu-role-palette-solver.cpp");
const BINARY = path.join(ROOT, "node_modules/.tmp/raw_webgpu_role_palette_solver");
const PLAN_PATH = resolveInputPath(readArg("--plan") ?? path.join(MANIFEST_DIR, `render_plan_${LEVEL_ID}.json`));
const INPUT_PATH = path.join(ROOT, "node_modules/.tmp", `raw_role_palette_metrics_${LEVEL_ID}.tsv`);
const TUNING_PATH = resolveInputPath(readArg("--tuning") ?? path.join(MANIFEST_DIR, `raw_role_palette_tuning_${LEVEL_ID}.json`));
const REPORT_PATH = resolveInputPath(readArg("--report") ?? path.join(MANIFEST_DIR, `raw_role_palette_optimizer_report_${LEVEL_ID}.json`));
...
console.log(
  `PASS raw WebGPU C++ role palette solver level=${LEVEL_ID} candidates=${summary.candidatesEvaluated} current=${summary.currentScore.toFixed(3)} best=${summary.bestScore.toFixed(3)} improvement=${summary.improvement.toFixed(3)}`,
);
console.log(`  plan=${path.relative(ROOT, PLAN_PATH)}`);
