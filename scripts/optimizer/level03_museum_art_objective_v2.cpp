...
struct Evaluation {
  ...
  double premiumMuseumStory = 0;
  double blackBrassDiscipline = 0;
  double hitTargetClarity = 0;
  ...
  double cheapScifiPenalty = 0;
  double hitTargetConfusionPenalty = 0;
};
...
static Candidate sample(const Target &t, uint64_t &familyState, uint64_t &caseState) {
  const bool isOrb = t.modelKey == "room_museum_color_orb_pedestal";
  const bool isBarrier = t.modelKey == "room_museum_low_barrier";
  const bool isRoute = t.modelKey == "room_museum_floor_route_inlay";
  const double premium = range(familyState, 0.02, 0.2);
  const double galleryQuiet = range(familyState, 0.02, 0.14);
  ...
  c.glass = jitter(caseState, t.glass + premium * 0.08, isBarrier || isRoute ? 0.08 : 0.18, 0.02, 0.96);
  c.stone = jitter(caseState, t.stone + galleryQuiet * 0.24, 0.14, 0.28, 1.0);
  c.brass = jitter(caseState, t.brass + 0.05 + range(familyState, -0.02, 0.08), 0.08, 0.06, 0.5);
  c.cyan = jitter(caseState, t.cyan - 0.08 + galleryQuiet * 0.12, isOrb ? 0.12 : 0.14, 0.02, isOrb ? 0.58 : 0.86);
  c.warmWhite = jitter(caseState, t.warmWhite + galleryQuiet * 0.26, 0.16, 0.12, 1.12);
  c.red = jitter(caseState, t.redMax * 0.24 + range(familyState, -0.004, 0.006), 0.025, 0.0, 0.12);
  ...
  c.roughness = jitter(caseState, t.roughness + range(familyState, -0.08, 0.03), 0.13, 0.2, 0.76);
...
static Evaluation evaluate(const Candidate &c, const Target &t) {
  ...
  const bool isRoute = t.modelKey == "room_museum_floor_route_inlay";
  const bool isBarrier = t.modelKey == "room_museum_low_barrier";
  e.blackBrassDiscipline = clamp01(
      0.3 * band(c.stone, 0.54, 1.0, 0.18) +
      0.26 * band(c.brass, 0.12, 0.42, 0.12) +
      0.2 * band(c.warmWhite, 0.28, 0.9, 0.18) +
      0.14 * band(c.cyan, 0.04, isOrb ? 0.52 : 0.72, 0.18) +
      0.1 * band(c.roughness, 0.22, 0.62, 0.16));
  e.premiumMuseumStory = clamp01(
      0.22 * e.museumAuthenticity +
      0.18 * e.blackBrassDiscipline +
      0.16 * e.materialLayering +
      0.15 * e.exhibitReadability +
      0.12 * e.roomLightHarmony +
      0.1 * band(c.detail, 0.64, 0.98, 0.16) +
      0.07 * band(c.reflection, isBarrier ? 0.04 : 0.12, isRoute ? 0.72 : 0.62, 0.18));
  e.hitTargetClarity = isOrb
      ? clamp01(0.24 * band(c.width, 0.68, 1.08, 0.16) + 0.18 * band(c.height, 0.86, 1.28, 0.16) + 0.18 * band(c.cyan, 0.22, 0.58, 0.14) + 0.16 * e.exhibitReadability + 0.14 * e.collisionFit + 0.1 * band(c.glass, 0.24, 0.58, 0.16))
      : 1.0;

  e.redConflictPenalty = clamp01(std::max(0.0, c.red - t.redMax) / 0.12);
  e.overGlowPenalty = clamp01(std::max(0.0, c.cyan + c.warmWhite - (isOrb ? 1.12 : 1.48)) / 0.55);
  ...
  e.cheapScifiPenalty = clamp01(
      0.34 * std::max(0.0, c.cyan - (isOrb ? 0.58 : 0.72)) / 0.36 +
      0.28 * std::max(0.0, 0.1 - c.brass) / 0.1 +
      0.22 * std::max(0.0, 0.48 - c.stone) / 0.48 +
      0.16 * std::max(0.0, c.roughness - 0.72) / 0.28);
  e.hitTargetConfusionPenalty = isOrb ? clamp01(std::max(0.0, c.cyan - 0.6) / 0.25 + std::max(0.0, c.glass - 0.72) / 0.28) : 0.0;
  ...
  if (e.premiumMuseumStory < 0.72) e.failures.push_back("NOT_PREMIUM_HUMAN_MUSEUM_STORY");
  if (e.blackBrassDiscipline < 0.64) e.failures.push_back("BLACK_BRASS_MATERIAL_DISCIPLINE_WEAK");
  if (isOrb && e.hitTargetClarity < 0.72) e.failures.push_back("ORB_HIT_TARGET_CLARITY_WEAK");
  e.pass = e.failures.empty();

  const double positive =
      0.1 * e.museumAuthenticity +
      0.09 * e.restrainedHorror +
      0.11 * e.exhibitReadability +
      0.08 * e.contactShadow +
      0.09 * e.roomLightHarmony +
      0.09 * e.materialLayering +
      0.09 * e.textureVisibility +
      0.07 * e.collisionFit +
      0.06 * e.configReuse +
      0.08 * e.premiumHardSurface +
      0.09 * e.premiumMuseumStory +
      0.04 * e.blackBrassDiscipline +
      0.01 * e.hitTargetClarity;
  const double penalty =
      ...
      0.16 * e.atlasPenalty +
      0.18 * e.cheapScifiPenalty +
      0.12 * e.hitTargetConfusionPenalty;
...
static void writeEvaluation(std::ostream &out, const Evaluation &e) {
  ...
  out << ",\"premiumMuseumStoryScore\":" << num(e.premiumMuseumStory);
  out << ",\"blackBrassDisciplineScore\":" << num(e.blackBrassDiscipline);
  out << ",\"hitTargetClarityScore\":" << num(e.hitTargetClarity);
  out << "},\"penalties\":{";
  ...
  out << ",\"cheapScifiPenalty\":" << num(e.cheapScifiPenalty);
  out << ",\"hitTargetConfusionPenalty\":" << num(e.hitTargetConfusionPenalty);
  out << "},\"failures\":[";
...
int main(int argc, char **argv) {
  ...
  const std::string outPath = argString(args, "out", "src/assets/manifests/reports/human_protocol_level03_museum_art_objective_v2_report.json");
  const uint64_t seed = static_cast<uint64_t>(argInt(args, "seed", 20260606));
  ...
  out << "{\n";
  out << "  \"schema\":\"human-protocol/level03-museum-art-objective@2\",\n";
  out << "  \"generatedAt\":" << js(timestamp()) << ",\n";
  out << "  \"objective\":\"AGE v2 museum objective: premium black-stone/aged-brass Human Museum exhibit furniture with story clarity, restrained cyan, hit-readable puzzle orb pedestals, image2 texture visibility, contact grounding, collision fit, and room-light harmony.\",\n";
  ...
  out << "  \"scoringTerms\":[\"museumAuthenticityScore\",\"restrainedHorrorScore\",\"exhibitReadabilityScore\",\"contactShadowScore\",\"roomLightHarmonyScore\",\"materialLayeringScore\",\"textureVisibilityScore\",\"collisionProxyFitScore\",\"configReuseScore\",\"premiumHardSurfaceScore\",\"premiumMuseumStoryScore\",\"blackBrassDisciplineScore\",\"hitTargetClarityScore\"],\n";
  out << "  \"targets\":[\n";
  ...
  out << "  \"recommendation\":\"Use the v2 best candidates for Blender generation. Keep v1 furniture as owned WGPU Lab legacy assets, but ship v2 for Level 3 because it hard-gates premium museum story, black/brass discipline, and orb hit readability.\",\n";
  out << "  \"elapsedSeconds\":" << num(elapsed) << "\n";
  out << "}\n";
  std::cout << "Level03 museum objective v2 complete. bestAverage=" << (bestTotal / targets.size()) << " currentAverage=" << (currentTotal / targets.size()) << " elapsed=" << elapsed << "s\n";
}
