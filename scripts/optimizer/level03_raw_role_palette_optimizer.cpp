...
Eval evaluate(const std::vector<RoleMetrics> &metrics, const Params &p, uint64_t &state) {
  ...
  for (const RoleMetrics &role : metrics) {
    ...
    const double roleWeight = role.weight * (role.role == "glass_shell" ? 1.35 : role.role == "route_gold" || role.role == "danger_red" ? 1.18 : 1.0);
...
void writeJson(const std::string &path, const Eval &best, const Eval &current, uint64_t candidatesEvaluated, const char *kind) {
  ...
  out << "  \"algorithm\": \"cpp-final-frame-role-palette-solver-v2\",\n";
  out << "  \"source\": \"scripts/optimizer/level03_raw_role_palette_optimizer.cpp\",\n";
  out << "  \"kind\": \"" << kind << "\",\n";
...
int main(int argc, char **argv) {
  try {
    if (argc < 6) {
      std::cerr << "usage: level03_raw_role_palette_optimizer <input.tsv> <tuning.json> <report.json> <families> <candidates>\n";
      return 2;
    }
